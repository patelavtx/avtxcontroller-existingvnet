...
# ADD1  -  can add variables here to use in main module
#***************** data to retrieve existing public ip ****************
data "azurerm_public_ip" "origctl" {
  name                = "orig-ctl-public-ip"                          # UPDATE with name and RG of original controller ip
  resource_group_name = "orig-ctl-rg"
}
...
resource "azurerm_network_interface" "aviatrix_controller_nic" {
  ...
  ip_configuration {
    ...
    # public_ip_address_id          = data.azurerm_public_ip.origctl.id             # ADD2 use origctl ip
  }
}
...
# UPDATED -  unique os-disk naming in same RG line 119
